...
const createMockResponse = (searchTerm: string = ''): APIResponse => {
  const filteredCharacters = searchTerm
    ? MOCK_CHARACTERS.filter((char) => char.name.toLowerCase().includes(searchTerm.toLowerCase()))
    : MOCK_CHARACTERS;
...
export class APIService {
  static async searchCharacters(searchTerm: string = '', retryCount = 0): Promise<APIResponse> {
    try {
      const url = searchTerm ? `${BASE_URL}/people/?search=${encodeURIComponent(searchTerm)}` : `${BASE_URL}/people/`;
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
          ...
          console.warn('Network error: Unable to connect to the Star Wars API. Using mock data as fallback.');
          return createMockResponse(searchTerm);
        }

        if (error.message.includes('CORS')) {
          console.warn('CORS error: Unable to access the Star Wars API. Using mock data as fallback.');
          return createMockResponse(searchTerm);
        }
